...
public function index(){
	...
	$version = getConfig("SynVersion");
	$data = array("key"=>$syn_key,"version"=>$version); //构造同步数据
	$menu_return = object_array(json_decode(curl_request($syn_url,$data)));
	//dump($menu_return);
	Log::Write($menu_return["status"]);
	if($menu_return["status"] == 1) {
		...
		//更新菜谱
		Log::Write("22222222222");
		$menus = $menu_return["cook_menu"];	
		Log::Write("333333333:".$menus);
		foreach ($menus as $key => $value) {
			...
			Log::Write("IS_DElETE:".$menu["is_delete"]);
...
public function updateAd()
{
	$cloud_ip=getConfig("cloud_ip");
	$syn_key=getConfig("synKey");
	$adsyn_url="http://".trim($cloud_ip)."/Cloud/SynData/Ads/synAd";
	$ad_version=getConfig("adSynVersion");
	if(empty($syn_key)||$ad_syn_url="")
	{
		$this->error("同步密钥为空，不能进行同步！");
	}
	$data = array('key' => $syn_key,'version'=>$ad_version );
	$ad_return=object_array(json_decode(curl_request($adsyn_url,$data)));
	if($ad_return['status']==1)
	{	
		$this->setConfig('adSynVersion',$ad_return['curversion']);
		$adlist=$ad_return['data'];
		foreach ($adlist as $key => $value) {
			# code...
			$condition['adnum']=$value['adnum'];
			$adinfo=$value;
			$ret=M("ads")->where($condition)->find();
			if($ret!=NULL)
			{
				unset($adinfo["adnum"]);
				M("ads")->where($condition)->save($adinfo);
	...
	elseif($ad_return["status"]==2)
	{
		Log::Write("没有要更新的广告数据");
	}
	elseif($ad_return["status"]==0)
	{
		Log::Write("syn_key不能为空");
	}
	else return;
	...
	public function setConfig($conf,$value){
		$val['val']=$value;
		$config=M('config')->where("conf=\"".$conf."\"")->save($val);
		return $config;
	}
}
