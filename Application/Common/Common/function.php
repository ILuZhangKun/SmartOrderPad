...
// 移植于common.php
    ...
    */
    ...
    function SaveConfig($conf,$data){
        // return $data;die;
        $ret = M('config')->where(array("conf"=>$conf))->save($data);
        return M("config")->getLastSql();
    }
    /**
    ...
    function getConfig($conf){
        $config = M('config')->where(array("conf"=>$conf))->find();
        return $config['val'];
    }
    // 把该函数去掉！！！
     ...
     * curl执行请求云端API
     */
    function curl_request($url,$post_data,$timeout=5){
        $ch = curl_init();
...
//	curl_setopt($ch, CURLOPT_RETURNTRANSFER, false);
    ...
    function curl_request_get($url,$data,$timeout=5){
	$ch = curl_init();
        $url = $url."/version/{$data['version']}/key/{$data['key']}";
	dump($url);
	curl_setopt ($ch, CURLOPT_URL, $url);
        curl_setopt ($ch, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt ($ch, CURLOPT_CONNECTTIMEOUT, $timeout);
    ...
    function getComIdBySynKey($syn_key){
        M("dept")->where("syn_key = '{$syn_key}'");

    }



    function image_download($remote,$local) {
        $ch = curl_init ();  
