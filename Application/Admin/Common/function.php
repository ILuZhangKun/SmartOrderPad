...
function buildString($data){
    if(is_array($data["_"])) $data = $data["_"];
    foreach ($data as $key => $value) {
        if(is_array($value["_"])){
        foreach ($value["_"] as $k => $v) {
            $temp[] = $v["title"];
        }
        $tree[$value["title"]] = $temp;
        unset($temp);
        }
    }
    return $tree;
 ...
 * 删除车辆："is_delete"
 * 所有车辆：0（包括逻辑删除和报废车辆）
 * 正常车辆：1（不包括逻辑删除和报废的车辆）
 */
function countCar($type,$val){
    if($type !== 0 && $type!==1)
        $condition = array("{$type}"=>"{$val}");
    elseif($type==1)
        $condition = array("is_delete"=>0,"is_scrap"=>0);
    elseif($type==0)
        $condition = "";
    else return "参数错误！";
    return M("car")->where($condition)->count();

}
/**
 * [countRemindNum 返回各种状态的车辆的数目]
 * Powered By BJUT
 * @copyright [MrXiao]
 * @license   [@BJUT]
 * @version   [0.1]
 ...
 */
function countRemindNum($type){
    $remindCar = D("Exam");
    return $remindCar->countNum($type);
}
