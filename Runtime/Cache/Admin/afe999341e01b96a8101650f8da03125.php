...
<body>
    ...
    <div id="main-content">
        ...
        <tbody>
            <?php if(is_array($CookStyle)): foreach($CookStyle as $key=>$v): ?><tr>
            <td><?php echo ($v["id"]); ?></td>
            <td><?php echo ($v["stylename"]); ?></td>
            <td><?php echo (getStyleCount($v["id"])); ?></td>
            <td name=<?php echo ($id = $v["id"]); ?>><a href="<?php echo U('/MenuManage/ModifyCookStyle/styleid/'.$id,'','');?>">修改</a> | 
            <a href="<?php echo U('/MenuManage/DelCookStyle/id/'.$id,'','');?>">删除</a></td>
