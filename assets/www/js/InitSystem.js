...
console.log('初始化函数中创建websocket');
    var ws = new WebSocket('ws://140.143.190.196:8000');
//    var ws = new WebSocket("ws://10.18.18.75:8000");
    ws.onopen = function() {
        console.log('websocket已连接');
        $('#toCallWaiter').click(function() {
            var tableName = localStorage.tableName;
            console.log(tableName);
            for (var i = modelCon.callWaiterOption.length - 1; i >= 0; i--) {
                if (modelCon.callWaiterOption[i].checked) {
                    console.log(modelCon.callWaiterOption[i].option);
                    ws.send(tableName+' '+modelCon.callWaiterOption[i].option);
                }
            }
            ringObj.callWaiter(modelCon.callWaiterOption);
        });
        $('#toStopCallWaiter').click(function() {
    ...
    ws.onerror = function(event) {
        console.log('websocket有错:' + event.err);
    }
}